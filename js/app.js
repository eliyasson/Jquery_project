...
$(document).ready(function() {
    ...
    function init() {
      ...
      // If no cities are saved, load weather data for Helsinki City; 
      if (cities.length === 0) {
        getWeather("Helsinki");
    ...
    function displayCurrentWeather(data) {
        ...
        var tempCelsius = ((data.current.main.temp - 32) * 5/9).toFixed(0); // Conversion formula: (°F - 32) * 5/9
        $("#temperature").text(`${tempCelsius}\u00B0 C`); // Display temperature in Celsius
    ...
    function displayForecast(data) {
      ...
      $.each(forecast, function(i, day) {
        ...
        var date = dayjs(day.dt_txt).format("ddd, MMM. D"); // Format example: Sun, Apr. 17
        var year = dayjs(day.dt_txt).format("YYYY");

        // Get the name of the day
        var dayName = dayjs(day.dt_txt).format("dddd");
    ...
    function createForecast(data) {
        ...
        if (firstResult.hour === 6) {
            for (var i = 10; i < forecastData.length; i += 8) {
                ...
                forecast.main.temp = convertToFahrenheit(forecast.main.temp);
    ...
    function convertToFahrenheit(tempFahrenheit) {
        return ((tempFahrenheit - 32) * 5 / 9).toFixed(0); // Convert Fahrenheit to Celsius
    }
